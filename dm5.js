(function () {
    ...
    if (!document.getElementById('typingmindSidebarFixMerged')) {
        ...
        sidebarStyle.innerHTML = `
            ...
            [data-element-id="new-chat-button-in-side-bar"] { background-color: ${CONFIG.colors.background} !important; color: ${CONFIG.colors.text} !important; }
            ...
            [data-element-id="selected-chat-item"] { background-color: ${CONFIG.colors.background} !important; }
    ...
    mainStyle.textContent = `
        [data-element-id="chat-space-beginning-part"] {
            ...
            border-bottom: 1px solid ${CONFIG.colors.border} !important; /* Optional: Add border if needed */
        }
        ...
        Utils.safe(() => {
            ...
            res = res.replace(
                ...
                (_, lang, code) => {
                    ...
                    // Ensure text color is applied to the pre block itself and code inside
            ...
            res = res.replace(
                /`([^`]+)`/g,
                (_, inline) =>
                    // Ensure text color is applied here too
            ...
            // Handle 'inline code' (alternative syntax used in the script)
            res = res.replace(
                /&#039;([^&#]+)&#039;/g,
                (_, content) =>
                    // Ensure text color is applied here too
