(function () {
    const CONFIG = {
        colors: {
            background: '#FFFCF9',    // Very light cream/off-white background
            text: '#111111',          // Nearly black text for strong contrast
            muted: '#666666',         // Medium gray for secondary text
            border: '#E5E0DB',        // Light beige/tan border color
            input: {
            ...
            button: {
                primary: '#111111',    // Black buttons for contrast
                hover: '#333333',      // Slightly lighter on hover
            },
            accent: {
                primary: '#111111',    // Primary accent color (black)
                secondary: '#666666',  // Secondary accent (gray)
                tertiary: '#F5F0EB',   // Very light beige for subtle highlights
            },
            panels: '#FFFFFF',         // White for panels/cards
            shadow: 'rgba(0, 0, 0, 0.06)', // Very subtle shadow color
            messageBg: '#F5F0EB',      // Light beige for message bubbles
            codeBg: '#F5F0EB',         // Light beige for code blocks
        },
    ...
    if (!document.getElementById('typingmindSidebarFixMerged')) {
        ...
        sidebarStyle.innerHTML = `
            ...
            [data-element-id="new-chat-button-in-side-bar"] { background-color: ${CONFIG.colors.button.primary} !important; color: white !important; border-radius: ${CONFIG.borderRadius.small} !important; box-shadow: none !important; }
            [data-element-id="new-chat-button-in-side-bar"] * { color: white !important; }
            [data-element-id="new-chat-button-in-side-bar"]:hover { background-color: ${CONFIG.colors.button.hover} !important; }
            [data-element-id="search-chats-bar"] { background-color: ${CONFIG.colors.input.background} !important; color: ${CONFIG.colors.text} !important; border: 1px solid ${CONFIG.colors.border} !important; border-radius: ${CONFIG.borderRadius.small} !important; box-shadow: none !important; }
            ...
            [data-element-id="selected-chat-item"] { font-weight: 500 !important; }
            ...
            #headlessui-portal-root [role="menu"] { display: block !important; visibility: visible !important; background-color: ${CONFIG.colors.panels} !important; color: ${CONFIG.colors.text} !important; pointer-events: auto !important; border-radius: ${CONFIG.borderRadius.small} !important; box-shadow: ${CONFIG.shadows.medium} !important; }
            ...
            [data-element-id="side-bar-background"] textarea:focus { background-color: ${CONFIG.colors.input.background} !important; color: ${CONFIG.colors.text} !important; border-color: ${CONFIG.colors.accent.primary} !important; box-shadow: 0 0 0 2px rgba(17,17,17,0.1) !important; }
            ...
            [data-element-id="workspace-profile-button"]:hover { background-color: rgba(17,17,17,0.05) !important; }
    ...
    inputStyle.textContent = `
        ...
        #chat-input-textbox:focus {
            ...
            box-shadow: 0 0 0 1px ${CONFIG.colors.button.primary} !important;
        }
        ...
        Utils.safe(() => {
            ...
            res = res.replace(
                ...
                (_, lang, code) => {
                    ...
                    }; overflow-x:auto; margin:16px 0; box-shadow:none;" class="code-block${
                        lang ? ' language-' + lang : ''
            ...
            res = res.replace(
                /`([^`]+)`/g,
                (_, inline) =>
                    `<code style="background-color:${CONFIG.colors.codeBg}; color:${CONFIG.colors.text}; padding:0.2em 0.4em; border-radius:${CONFIG.borderRadius.small}; font-family:'SF Mono', Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace; font-size:13px; border:1px solid ${CONFIG.colors.border};">${inline}</code>`
            );
            res = res.replace(
                /&#039;([^&#]+)&#039;/g,
                (_, content) =>
                    `<code style="background-color:${CONFIG.colors.codeBg}; color:${CONFIG.colors.text}; padding:0.2em 0.4em; border-radius:${CONFIG.borderRadius.small}; font-family:'SF Mono', Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace; font-size:13px; border:1px solid ${CONFIG.colors.border};">${content}</code>`
            );
            return res;
        }, 'multiStepParse');
    ...
    const processMessageContent = safeTxt =>
        Utils.safe(() => {
            ...
            tests.forEach(({ open, inner, close }, i) => {
                ...
                proc = proc.replace(
                    `__TEST_${i}__`,
                    `${open}<code style="background-color:${CONFIG.colors.codeBg}; color:${CONFIG.colors.text}; padding:0.2em 0.4em; border-radius:${CONFIG.borderRadius.small}; font-family:'SF Mono', Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace; font-size:13px; border:1px solid ${CONFIG.colors.border};">${parsed}</code>${close}`
                );
            });
            return proc;
            ...
            Utils.safe(() => {
                ...
                globalFix.textContent = `
                    * {
                    ...
                    h1 { font-weight: 600 !important; letter-spacing: -0.01em !important; font-size: 1.5rem !important; }
                    h2 { font-weight: 600 !important; letter-spacing: -0.01em !important; font-size: 1.3rem !important; }
                    h3 { font-weight: 500 !important; letter-spacing: -0.01em !important; font-size: 1.15rem !important; }
                    ...
                    /* Improve blockquotes */
                    blockquote {
                        border-left: 4px solid ${CONFIG.colors.button.primary} !important;
                        ...
                        border-radius: 0 ${CONFIG.borderRadius.small} ${CONFIG.borderRadius.small} 0 !important;
                    }
                    ...
                    hr {
                        ...
                        background: ${CONFIG.colors.border} !important;
    ...
    console.log('Anthropic-inspired clean minimalist theme applied');
})();
