(function () {
        ...
        Utils.safe(() => {
            ...
            res = res.replace(
                ...
                (_, lang, code) => {
                    ...
                    return `<pre style="background:${CONFIG.colors.background}; border:1px solid ${CONFIG.colors.border}; padding:6px; border-radius:${CONFIG.borderRadius.small}; overflow-x:auto; margin:0;" class="code-block${lang ? ' language-' + lang : ''}"><code style="white-space:pre; display:block; overflow-wrap:normal; word-break:normal;">${code}</code></pre>`;
                }
            );
            res = res.replace(
                /`([^`]+)`/g,
                (_, inline) =>
                    `<code style="background-color:#2e2e2e; color:${CONFIG.colors.text}; padding:0.2em 0.4em; border-radius:3px;">${inline}</code>`
            );
            res = res.replace(
                /&#039;([^&#]+)&#039;/g,
                (_, content) =>
                    `<code style="background-color:#2e2e2e; color:${CONFIG.colors.text}; padding:0.2em 0.4em; border-radius:3px;">${content}</code>`
            );
            return res;
        }, 'multiStepParse');
    ...
    const processMessageContent = safeTxt =>
        Utils.safe(() => {
            ...
            tests.forEach(({ open, inner, close }, i) => {
                ...
                proc = proc.replace(
                    `__TEST_${i}__`,
                    `${open}<code style="background-color:#2e2e2e; color:${CONFIG.colors.text}; padding:0.2em 0.4em; border-radius:3px;">${parsed}</code>${close}`
                );
            });
            return proc;
    ...
    console.log('typingmind-custom-dark.js: Loaded.');
})();
